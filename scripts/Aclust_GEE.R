...
clusters.list.all = clusters.all

max_1st <- 2
clusters.list.all_ <- list()
for (i in 1:length(clusters.all)) {
  if (length(clusters.list.all[[i]]) > 1) {
    if (i == 1) 
      clusters.list.all_ <- clusters.list.all[1]
    else clusters.list.all_ <- c(clusters.list.all_, clusters.list.all[i])
    if (length(clusters.list.all[[i]]) > max_1st)
      max_1st = length(clusters.list.all[[i]])
  }
}
...
GEE.results.clusters <- GEE.clusters(betas, clusters.list.all, exposure, id = colnames(betas), working.cor = "ex")
